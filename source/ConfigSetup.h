...
EMP_BUILD_CONFIG(SymConfigBase,
    ...
    VALUE(SEED, int, 10, "What value should the random seed be? Each random seed will result in a distinct experiment. If the seed is -1, then it is randomly re-chosen."),
    ...
    VALUE(MUTATION_RATE, double, 1, "What is the chance of a mutation occurring? The range is 0 to 1, where 1 means the mutation occurs every time a host/symbiont reproduces and vice versa."),
    ...
    VALUE(RES_DISTRIBUTE, int, 100, "During each update, each host will receive this number of resources. Usually between 10-100."),
    ...
    VALUE(GRID, bool, 0, "Do offspring get placed immediately next to their parents upon reproduction? If grid is off, offspring will be placed in a random location."),
