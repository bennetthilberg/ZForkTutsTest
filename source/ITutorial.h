...
class ITutorial {
    ...
    public:
        ...
        ITutorial(UI::Document & animation, UI::Document & settings, UI::Document & explanation, UI::Document & learnmore, UI::Document & buttons, UI::Document & top_bar, UI::Canvas & mycanvas,UI::Document & instructions, UI::Button & my_button): doc("emp_base"), ok_but([](){}, "OK"){
            ...
            /*reset_but_state*/
            tut.AddPopoverEffect("reset_but_state", buttons, "To reset the simulation, push reset. You will need to click reset after you make changes to the settings before starting the simulation.", ok_but, "1vh", "0vw", "auto");
            tut.AddOverlayEffect("reset_but_state", buttons, settings, "black", 0.8, 10, true);
            /*settings_state*/
            tut.AddPopoverEffect("settings_state", buttons, "These are the settings panels. Each setting has a dropdown that explains more about what that setting does. Try expanding a dropdown to learn more about a setting. ", ok_but, "-7vh", "-27vw", "auto");
            tut.AddOverlayEffect("settings_state", buttons, buttons, "black", 0.8, 10, true);
 
            /*settings_change_state*/
            tut.AddPopoverEffect("settings_change_state", buttons, "You can change a setting by adjusting the slider, typing in a value, or using the small arrows. Try changing a setting now!", ok_but, "-7vh", "-27vw", "auto");
            tut.AddOverlayEffect("settings_change_state", buttons, settings, "black", 0.8, 10, true);
 
            /*reset_change_state*/
            tut.AddPopoverEffect("reset_change_state", buttons, "After changing your settings, you must push reset to see your changes reflected in the petri dish. Try pushing reset, then starting the simulation again. Did you see the changes?", ok_but, "-7vh", "-27vw", "auto");
            tut.AddOverlayEffect("reset_change_state", buttons, settings, "black", 0.8, 10, true);
            
            /*lab_instruct_state*/
            tut.AddPopoverEffect("lab_instruct_state", buttons, "Your lab instructions are held here. Each section can be collapsed to save space on your screen", ok_but, "-7vh", "30vw", "auto");
            tut.AddOverlayEffect("lab_instruct_state", buttons, explanation, "black", 0.8, 10, true);
 
            /*graph_state*/
            tut.AddPopoverEffect("graph_state", buttons, "Below the lab instructions are live graphs. Click the drop down to see them!", ok_but, "1vh", "30vw", "auto");
            tut.AddPopoverEffect("graph_state", buttons, "Click reset and start on the lab to watch the graphs update in real time. ", ok_but, "1vh", "30vw", "auto");
            tut.AddOverlayEffect("graph_state", buttons, explanation, "black", 0.8, 10, true);
 
            /*repeat_state*/
            tut.AddPopoverEffect("repeat_state", buttons, "You have finished the tutorial! To start it again at any time, click the start tutorial button. If you have any questions, click the FAQ button, or refer to the symbulation background. ", ok_but, "1vh", "30vw", "auto");
